...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    # Forward pass
    h0 = np.dot(features, W_proj) + b_proj  # h0 shape = N X H
    x, cache_word_embed = word_embedding_forward(captions_in, W_embed)  # x shape = N X T x W

    forward_net = {'lstm': lstm_forward, 'rnn': rnn_forward}[self.cell_type]
    h, cache_nn = forward_net(x, h0, Wx, Wh, b)

    scores, cache_affine = temporal_affine_forward(h, W_vocab, b_vocab) # scores shape N x T x V
    ...
    dx, dh0, dWx, dWh, db = backward_net(dh, cache_nn)
  ...
  def sample(self, features, max_length=30):
    ...
    word_idxs = np.full(N, self._start)
    prev_h = h0
